...
class Net(nn.Module):
    def __init__(self):
        ...
        # common used Prelu是带参数的relu,随之更新
        ...
        self.ave_pool = nn.AvgPool2d(2, 2, ceil_mode=True)#均值池化
...
def train(args, train_loader, valid_loader, model, criterion, optimizer, device):
    ...
    ######################
        ...
        with torch.no_grad():
            ...
            valid_mean_pts_loss))
            if valid_mean_pts_loss < threshold_loss:
                threshold_loss = valid_mean_pts_loss
                print('best_model:{:.6%}'.format(threshold_loss))

        print('====================================================')
# save model
    if args.save_model:
        saved_model_name = os.path.join(args.save_directory, 'detector_epoch' + '_'  + '.pt')
